...
export async function shareOnWhatsApp(bill: Bill, template?: TemplateSettings): Promise<void> {
  ...
  try {
    ...
    if (pdfBlob && navigator.share && navigator.canShare) {
      ...
      const pdfFile = new File([pdfBlob], `invoice-${bill.customerName.replace(/\s+/g, '-')}-${bill.id}.pdf`, {
        type: 'application/pdf'
      });
      
      const shareData = {
        title: `Invoice from ${template?.companyName || 'AP1700'} - ${bill.customerName}`,
  ...
  // Fallback to text sharing with WhatsApp link
  const message = `
🧾 *Invoice from ${template?.companyName || 'AP1700'}*
...
📧 *Payment Details:*
• Due Date: ${formatDate(new Date(new Date(bill.date).getTime() + (template?.dueDays || 30) * 24 * 60 * 60 * 1000).toISOString())}
• Payment Terms: Net ${template?.dueDays || 30} days
• Include Invoice # with payment

📞 *Contact Information:*
${template?.companyEmail ? `• Email: ${template.companyEmail}` : '• Email: contact@ap1700.com'}
${template?.companyPhone ? `• Phone: ${template.companyPhone}` : '• Phone: +1 (555) 123-4567'}
${template?.companyWebsite ? `• Website: ${template.companyWebsite}` : '• Website: www.ap1700.com'}

Generated by *${template?.companyName || 'AP1700'} Professional Bill Management System*

_Note: PDF invoice will be shared separately due to platform limitations._
  `.trim();
...
export async function shareViaWebShare(bill: Bill, template?: TemplateSettings): Promise<void> {
  ...
  // Fallback to text sharing without files
  const shareData = {
    title: `Invoice from ${template?.companyName || 'AP1700'} - ${bill.customerName}`,
    text: `Invoice #${bill.id.toUpperCase()} for ${bill.customerName} - Total: ₹${bill.total.toFixed(2)} - Date: ${formatDate(bill.date)}`,
